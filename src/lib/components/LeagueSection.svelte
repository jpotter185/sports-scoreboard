<script lang="ts">
  ...
  $: try {
    if (typeof window !== 'undefined') localStorage.setItem(storageKey, expanded ? '1' : '0');
  } catch {}
  ...
  <div class="league-header">
    <div class="header-top">
      ...
      <div class="right-controls">
        <FavoriteButton isFavorite={leagueIsFavorite} size="medium" on:toggle={toggleFavorite} />
        {#if reorderMode}
        ...
        <button
          ...
          on:click={() => {
            if (reorderMode) dispatch('exitReorder');
            dispatch('clearJump');
            toggleCollapse();
          }}
          aria-expanded={!expanded}
          aria-controls={`grid-${league.id}`}
        >
          <svg
            class="chevron"
    ...
    <div class="league-meta">
      <div class="meta-item">
        <span class="meta-icon"
          >{league.id === 'nfl' ? '🏈' : league.id === 'mlb' ? '⚾' : '⚽'}</span
        >
