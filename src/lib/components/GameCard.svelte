<script lang="ts">
  ...
  $: awayTeamInternalId = leagueId
    ? `${leagueId}-${game.awayTeam.id}`
    : `unknown-${game.awayTeam.id}`;
  $: homeTeamInternalId = leagueId
    ? `${leagueId}-${game.homeTeam.id}`
    : `unknown-${game.homeTeam.id}`;
...
<div class="game-card" class:live={isLive} class:final={isFinal} class:scheduled={isScheduled}>
  ...
  <div class="teams-container">
    ...
    <div
      ...
      class:losing-team={isFinal &&
        (game.awayScore < game.homeScore || game.awayScore === game.homeScore)}
  ...
  <a
    href="/game/{game.id}?backTo={backTo}"
