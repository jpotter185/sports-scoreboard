...
function convertESPNGame(
  espnGame: ESPNGame,
  sport: 'football' | 'soccer' | 'baseball',
  league: 'nfl' | 'mls' | 'epl' | 'mlb'
): Game {
  ...
  if (espnGame.status.type.state === 'pre') {
    ...
    if (sport === 'football') {
      ...
      period =
        typeof outs === 'number'
          ? `${baseLabel} - ${outs} ${outs === 1 ? 'Out' : 'Outs'}`
          : baseLabel;
    }
...
export async function fetchNFLGames(
  ...
  try {
    ...
    if (season) {
      params.append('year', season.toString());
    }
    if (params.toString()) {
      url += '?' + params.toString();
    }
...
export async function fetchNFLTeams(fetchFn?: typeof globalThis.fetch): Promise<Team[]> {
  try {
    ...
    const teamsWithStats = await Promise.all(
      data.sports[0].leagues[0].teams.map(async espnTeam => {
        ...
        const winPercentage = detailedStats?.winPercent
          ? Math.round(detailedStats.winPercent * 1000) / 1000
          : undefined;
  ...
  try {
    ...
    return Promise.all(
      data.sports[0].leagues[0].teams.map(async espnTeam => {
        ...
        const conference =
          statsData.team.groups.id === '1' ? 'Eastern Conference' : 'Western Conference';
      ...
      teams.map(async espnTeam => {
        ...
        if (detailedStats) {
          ...
          if (detailedStats.record && detailedStats.record.includes('-')) {
            const [w, l] = detailedStats.record
              .split('-')
          ...
          if (detailedStats.stats) {
            gamesBack =
              detailedStats.stats.find(
                (s: { name: string; value: number }) => s.name === 'gamesBehind'
              )?.value || 0;
            divisionGamesBehind =
              detailedStats.stats.find(
                (s: { name: string; value: number }) => s.name === 'divisionGamesBehind'
              )?.value || 0;
          }
        ...
        const winPercentage =
          totalGames > 0 ? Math.round((parsedWins / totalGames) * 1000) / 1000 : 0;
